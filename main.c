...
//Check if a generic node is connected
//@param graph [graph structure]
//@param target_node  [node to check]
//@param nodes [nodes in graph]
boolean is_connected_node(byte_t** graph, int target_node, int nodes);

//Print a graph as matrix
//@param graph [graph structure]
//@param nodes [nodes in graph]
void print_graph(byte_t** graph, int nodes);

//malloc wrapper
void* smart_zmalloc(size_t bytes);
...
int main(int argc, char** argv)
{
	...
	graph = (byte_t**)smart_zmalloc((nodes + sizeof(byte_t*)) * sizeof(byte_t*));

	for (x = 0; x < nodes; ++x)
		graph[x] = (byte_t*)smart_zmalloc(nodes * sizeof(byte_t) + sizeof(byte_t));
...
boolean is_connected_node(byte_t** graph, int target_node, int nodes)
{
	...
	stack = (byte_t*)smart_zmalloc(nodes * sizeof(char) + sizeof(char));
	visited = (byte_t*)smart_zmalloc(nodes * sizeof(char) + sizeof(char));
	stack_interrupt = (byte_t*)smart_zmalloc(nodes * sizeof(char) + sizeof(char));
